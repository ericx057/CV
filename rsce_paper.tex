...
We evaluate RSCE on multi-document question answering (LongBench) and cross-file code completion (RepoBench-C) across four decoder-only architectures ranging from 7B to 24B parameters. On QA tasks using LLaMA-3.1 8B ($n = 108$), Vec+F (F1 = 0.333) retains 81\% of the full-context baseline (F1 = 0.410) while reducing input tokens by $\mathbf{99.4\%}$. A question-only ablation confirms that the dual-channel design is necessary: vector injection alone (F1 = 0.252) suppresses parametric memory below the Q-only level (F1 = 0.286), but Vec+F substantially recovers quality, establishing that the latent vector and fact block are complementary channels. On RepoBench-C across all four models ($n = 200$ per model, 800 total), Vec+F retains a mean Edit Similarity of 0.345 against a 0.370 baseline ($\Delta = -0.025$, $p < 10^{-5}$) while achieving $\mathbf{81.2\%}$ input token reduction. Our results suggest a correlation between parameter count and compression tolerance: larger models exhibit less quality degradation under residual stream injection, indicating that RSCE scales favorably with model capacity. We further demonstrate that optimal fact block strategies are architecture-dependent, and that the Exact Match degradation is a deliberate trade-off in which RSCE sacrifices verbatim string generation in exchange for massive token savings and localized semantic precision.
\end{abstract}
...
We run inference on four instruction-tuned decoder architectures using bfloat16 precision via the HuggingFace Transformers framework on NVIDIA H100 GPUs: LLaMA-3.1 8B, Qwen2.5 7B, DeepSeek-R1-Distill 14B, and Mistral Small 24B. RepoBench evaluation uses all four models; QA evaluation uses LLaMA-3.1 8B.
...
Table~\ref{tab:repobench} reports RepoBench-C Edit Similarity metrics ($n = 200$ per model, 800 total). Both Vec and Vec+F degrade statistically significantly relative to the full-context baseline (paired $t$-test: Vec $t = -4.38$, $p < 10^{-4}$; Vec+F $t = -4.58$, $p < 10^{-5}$; $n = 800$). Figure~\ref{fig:repobench_bars} provides per-model visualizations.

\begin{table}[h]
\centering
\caption{RepoBench-C results ($n = 200$ per model, 800 total). $\Delta$ = EditSim difference relative to same-model baseline.}
\label{tab:repobench}
...
\midrule
Average      & --- & 0.346 & 0.345 & $-0.024$ & $-0.025$ & 81.2\% \\
\midrule
\multicolumn{7}{l}{\textit{Baseline EditSim: LLaMA 0.348, Qwen 0.392, DeepSeek 0.344, Mistral 0.397}} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{axis}[
    ...
    ymin=0.28, ymax=0.44,
    xtick={1,2,3,4},
    xticklabels={LLaMA-3.1 8B, Qwen2.5 7B, DeepSeek 14B, Mistral 24B},
    legend style={at={(0.98,0.98)}, anchor=north east},
    title={RepoBench-C: Per-Model Edit Similarity ($n=200$ per model)},
    grid=major,
    grid style={dotted},
]
\addplot[fill=gray!20, draw=black] coordinates {(1,0.348)(2,0.392)(3,0.344)(4,0.397)};
\addplot[fill=gray!60, draw=black] coordinates {(1,0.317)(2,0.365)(3,0.322)(4,0.381)};
\addplot[fill=black!80, draw=black] coordinates {(1,0.319)(2,0.355)(3,0.330)(4,0.377)};
\legend{Baseline, Vec, Vec+F}
\end{axis}
\end{tikzpicture}
\caption{Edit Similarity per model across conditions. Mistral 24B displays maximal robustness under context compression ($\Delta\text{Vec} = -0.016$). Degradation narrows monotonically from 7--8B to 24B.}
...
\begin{axis}[
    ...
    grid=major,
    grid style={dotted},
    title={Compression Degradation vs.\ Model Size (RepoBench-C, $n=200$)},
]
% Vec data points
\addplot[
    only marks,
...
\addlegendentry{Vec ($R^2 = 0.922$)}

% Vec trend line: y = 0.01117*ln(x) - 0.05154
\addplot[thick, black, domain=5:32, samples=80] {0.01117*ln(x) - 0.05154};
\addlegendentry{}
...
\addlegendentry{Vec+F ($R^2 = 0.591$)}

% Vec+F trend line: y = 0.01428*ln(x) - 0.06026
\addplot[thick, gray, dashed, domain=5:32, samples=80] {0.01428*ln(x) - 0.06026};
\addlegendentry{}

% Model labels
\node[above right, font=\small] at (axis cs:7,-0.028)  {Qwen};
\node[above right, font=\small] at (axis cs:8,-0.031)  {LLaMA};
\node[above right, font=\small] at (axis cs:14,-0.022) {DeepSeek};
\node[above right, font=\small] at (axis cs:24,-0.016) {Mistral};
\end{axis}
\end{tikzpicture}
\caption{$\Delta$EditSim (degradation relative to full-context baseline) as a function of log parameter count for Vec (circles, solid trend) and Vec+F (squares, dashed trend). Vec degrades less as model size increases ($R^2 = 0.922$), suggesting a correlation between compression tolerance and parameter count. Vec+F shows a weaker but directionally consistent trend ($R^2 = 0.591$), reflecting architecture-dependent variation in how the fact block interacts with the latent vector.}
\label{fig:scaling_scatter}
\end{figure}

The Vec trend is strongly linear in log-parameter space ($R^2 = 0.922$), with degradation falling from $-0.031$ at 8B to $-0.016$ at 24B. The Vec+F trend is weaker ($R^2 = 0.591$), reflecting that the fact block introduces architecture-dependent effects that partially decorrelate from raw scale. Because we test four distinct architectures rather than a single model family at multiple scales, we interpret these correlations as suggestive rather than definitive: architecture and training regime co-vary with parameter count across our sample, and the trends should be validated with within-family scaling comparisons.
