...
% Vec trend line: y = 0.01117*ln(x) - 0.05154, R^2 = 0.922
\addplot[thick, black, domain=5:32, samples=80] {0.01117*ln(x) - 0.05154};
\addlegendentry{Vec trend ($R^2 = 0.922$)}
...
\end{axis}
\end{tikzpicture}
\caption{$\Delta$EditSim (degradation relative to full-context baseline) as a function of log parameter count. Vec (circles) shows a strong linear trend in log-parameter space ($R^2 = 0.922$), suggesting a correlation between model scale and compression tolerance. Vec+F (squares) is shown without a trend line as architecture-dependent variation in fact block effectiveness reduces the fit.}
\label{fig:scaling_scatter}
\end{figure}

Vec degradation is strongly linear in log-parameter space ($R^2 = 0.922$), falling from $-0.031$ at 8B to $-0.016$ at 24B. Vec+F does not exhibit a comparably clean trend, reflecting that the fact block introduces architecture-dependent effects that partially decorrelate from raw scale. Because we test four distinct architectures rather than a single model family at multiple scales, the Vec correlation is suggestive rather than definitive: architecture and training regime co-vary with parameter count, and the relationship should be validated with within-family scaling comparisons.
