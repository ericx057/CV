...
console = Console()
SUITE_RESULTS_DIR = results_path("ehpc_suite_results")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run EHPC LongBench and/or RepoBench sequentially across models")
    parser.add_argument(
        "--benchmarks",
        nargs="+",
    ...
    parser.add_argument("--pool-kernel", type=int, default=DEFAULT_POOL_KERNEL)
    ...
    parser.add_argument("--longbench-total", type=int, default=200)
    parser.add_argument("--longbench-max-tokens", type=int, default=80, dest="longbench_max_tokens")

    parser.add_argument("--repobench-n", type=int, default=200)
    parser.add_argument("--repobench-split", default="cross_file_first")
    parser.add_argument("--repobench-level", default="all")
    parser.add_argument(
        "--repobench-settings",
        nargs="+",
    ...
    parser.add_argument("--repobench-max-tokens", type=int, default=60, dest="repobench_max_tokens")
    return parser
...
def main() -> int:
    ...
    run_prefix = args.run_prefix or f"ehpc_suite_{int(time.time())}"

    console.rule("[bold magenta]EHPC Suite[/bold magenta]")
        ...
        for model_key in args.models:
            try:
                model_records = run_longbench_model(
                    ...
                    setting_names=args.longbench_settings,
        ...
        for model_key in args.models:
            try:
                model_records = run_repobench_model(
                    ...
                    setting_names=args.repobench_settings,
                    pool_kernel=args.pool_kernel,
