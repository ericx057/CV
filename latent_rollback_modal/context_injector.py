...
def generate_with_context_injection(
    ...
    ctx_tensor = ctx_v.to(device=wrapper.device, dtype=next(wrapper._model.parameters()).dtype)

    def injection_hook(h: torch.Tensor) -> torch.Tensor:
        return h + scale * ctx_tensor[None, None, :]
